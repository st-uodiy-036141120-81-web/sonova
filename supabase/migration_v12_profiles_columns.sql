-- Fix: record "v_row" has no field "verified"
-- Prefer running FIX_ONBOARDING.sql (all-in-one). This file kept for reference.
...
begin
  ...
  if trim(p_username) !~ '^[a-zA-Z0-9]{1,7}$' then raise exception 'Invalid username'; end if;
  if exists (select 1 from public.profiles where username = trim(p_username) and id <> v_uid) then
  ...
  return (
    select to_jsonb(p) - 'totp_secret' - 'daily_upload_count' - 'upload_count_date'
